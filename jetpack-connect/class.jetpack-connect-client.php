...
class Jetpack_Connect_Client {
	...
	public static function remote_request( $args, $body = null ) {
		...
		$token = Jetpack_Connect_Data::get_access_token( $args['user_id'] );
		if ( !$token ) {
		...
		$time_diff = (int) Jetpack_Connect_Options::get_option( 'time_diff' );
		$jetpack_signature = new Jetpack_Connection_Signature( $token->secret, $time_diff );
		...
		if ( false !== strpos( $args['url'], 'xmlrpc.php' ) ) {
			$url_args = array(
				'for'           => 'jetpack',
				'wpcom_blog_id' => Jetpack_Connect_Options::get_option( 'id' ),
			);
		} else {
			$url_args = array();
		}
		...
		$url = Jetpack_Connection::fix_url_for_bad_hosts( $url );
		...
		return Jetpack_Connect_Client::_wp_remote_request( $url, $request );
	}
	...
	public static function _wp_remote_request( $url, $args, $set_fallback = false ) {
		 ...
		 */
		if ( apply_filters( 'Jetpack_Connect_Client_verify_ssl_certs', false ) ) {
			return wp_remote_request( $url, $args );
		}

		$fallback = Jetpack_Connect_Options::get_option( 'fallback_no_verify_ssl_certs' );
		if ( false === $fallback ) {
			Jetpack_Connect_Options::update_option( 'fallback_no_verify_ssl_certs', 0 );
		}
		...
		||
			...
			Jetpack_Connect_Client::set_time_diff( $response, $set_fallback );
			return $response;
		}
		...
		if ( !is_wp_error( $response ) ) {
			...
			Jetpack_Connect_Options::update_option( 'fallback_no_verify_ssl_certs', time() );
			Jetpack_Connect_Client::set_time_diff( $response, $set_fallback );
		}
	...
	public static function set_time_diff( &$response, $force_set = false ) {
		...
		if ( $force_set ) { // during register
			Jetpack_Connect_Options::update_option( 'time_diff', $time_diff );
		} else { // otherwise
			$old_diff = Jetpack_Connect_Options::get_option( 'time_diff' );
	...
	static function wpcom_json_api_request_as_blog( $path, $version = self::WPCOM_JSON_API_VERSION, $args = array(), $body = null, $base_api_path = 'rest' ) {
		...
		$validated_args = array_merge( $filtered_args, array(
			'url'     => $url,
			'blog_id' => (int) Jetpack_Connect_Options::get_option( 'id' ),
			'method'  => $request_method,
		) );

		return Jetpack_Connect_Client::remote_request( $validated_args, $body );
	}
