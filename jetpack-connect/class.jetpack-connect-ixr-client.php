 ...
 *
 */
class Jetpack_Connect_IXR_Client extends IXR_Client {
	public $jetpack_args = null;

	function __construct( $args = array(), $path = false, $port = 80, $timeout = 15 ) {
		$defaults = array(
			'url' => Jetpack_Connect::xmlrpc_api_url(),
			'user_id' => 0,
		);
	...
	function query() {
		...
		$response = Jetpack_Connect_Client::remote_request( $this->jetpack_args, $xml );
 ...
 * @since 1.5
 */
class Jetpack_IXR_ClientMulticall extends Jetpack_Connect_IXR_Client {
	public $calls = array();
