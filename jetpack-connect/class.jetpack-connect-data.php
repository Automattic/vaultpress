...
class Jetpack_Connect_Data {
	...
	public static function get_access_token( $user_id = false ) {
		...
		if ( $user_id ) {
			if ( !$tokens = Jetpack_Connect_Options::get_option( 'user_tokens' ) ) {
				return false;
			}
			if ( $user_id === JETPACK_MASTER_USER ) {
				if ( !$user_id = Jetpack_Connect_Options::get_option( 'master_user' ) ) {
					return false;
				}
			}
	...
	/**
	 * This function mirrors Jetpack_Connect_Data::is_usable_domain() in the WPCOM codebase.
	 *
	 * @param $domain
	 * @param array $extra
	 *
	 ...
	 *
	 * This function mirrors Jetpack_Connect_Data::php_bug_66229_check() in the WPCOM codebase.
	 */
